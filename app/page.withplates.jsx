...
// ====== 新增：关联板块（行业）逻辑 ======
// 说明：仅在前端静态环境下运行，使用东财 push2 JSONP；失败会自动降级为不展示
try {
  // 1) 收集本轮更新基金的前10重仓股代码
  const allStockCodes = [];
  updated.forEach((f) => {
  ...
  if (allStockCodes.length > 0) {
    // 2) 构建 stock -> 行业板块 映射（带 localStorage 缓存、按需停止）
    const stockToPlates = await ensureStockToIndustryPlateMap(allStockCodes, {
      concurrency: 3,
      maxPlatesToScan: 200
    });

    // 3) 获取行业板块实时涨跌幅（全局缓存）
    const plateQuotes = await fetchIndustryPlateQuotes();

    // 4) 为每只基金聚合 Top3 关联板块，并写回 fund.relatedPlates
    updated.forEach((f) => {
    ...
    updated.forEach((f) => { f.relatedPlates = []; });
  }
} catch (e) {
  console.warn('关联板块计算失败（将降级为不展示）', e);
  updated.forEach((f) => { f.relatedPlates = []; });
}
// ====== 新增结束 ======
...
{/* 关联板块（行业） */}
{Array.isArray(f.relatedPlates) && f.relatedPlates.length > 0 && (
  <div
    className="row"
    style={{ marginBottom: 10, gap: 8, flexWrap: 'wrap', alignItems: 'center' }}
  >
    <span className="muted" style={{ fontSize: 12 }}>关联板块</span>
    {f.relatedPlates.map((p) => (
      <span
        key={`${f.code}-${p.name}`}
        className={`badge ${typeof p.pct === 'number'
          ? (p.pct > 0 ? 'up' : p.pct < 0 ? 'down' : '')
          : ''}`}
        title={`前10重仓中命中 ${p.count} 只`}
